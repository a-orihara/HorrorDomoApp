class Api::V1::Admin::UsersController < ApplicationController
  ...
  private
    ...
    def admin_user
      # 2.1 unless:指定された条件が偽（falseまたはnil）の場合に内部の処理を実行します
      unless current_api_v1_user&.admin?
        render json: {
          status: 'error',
          message: 'この操作は、管理者のみ行うことができます。'
          # 2.3 403 Forbidden 特定のリソースへのアクセス権限がない場合に使用
        }, status: :forbidden
      end
    end
end
...
================================================================================================
2.2
if文を使う場合も可能ですが、否定形を使用することになります。つまり、unlessを使用するときよりも少し読みづらくなる可
能性があります。
`if !current_api_v1_user&.admin?`
この場合、!current_api_v1_user&.admin? は current_api_v1_user&.admin? の否定形で、これが真の場合（現在の
ユーザーが管理者ではない、またはユーザーが存在しない場合）に内部のコードが実行されます。
