...
が設定されている。
set_user_by_tokenは、送られてきたトークンを用いてユーザーを特定し、@resource変数にそのユーザー情報をセットする。
したがって、このコントローラでは、before_actionに、@user = User.find(params[:id])のような処理は必要ありませ
ん。
その理由は、すでにset_user_by_tokenメソッドによって認証されたユーザーが特定され、そのユーザー情報が@resourceに
格納されているからです。
