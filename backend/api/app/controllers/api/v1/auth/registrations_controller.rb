...
class Api::V1::Auth::RegistrationsController < DeviseTokenAuth::RegistrationsController
  ...
  private
    ...
    # 2
    # sign_up_paramsは、Deviseが提供するメソッドで、サインアップ時に必要なパラメータを設定
    def sign_up_params
      # サインアップ時に登録できるカラムを指定
...
------------------------------------------------------------------------------------------------
.resource
Deviseによって定義されるヘルパーメソッドで、コントローラー内で利用できるユーザーモデルのインスタンスを返します。
.resource.avatar.attach
このヘルパーメソッドが返すユーザーモデルのインスタンスに紐づいたActive Storageの avatar アタッチメント（プロパ
ティ）に、引数で渡されたファイルをアタッチするためのメソッドです。
avatarはUserモデルで定義されたActive Storageの添付ファイル（attachment）であり、また添付されたファイルを操作
するためのプロパティです。

resource.avatar.attach(account_update_params[:avatar])
取得したファイル情報を、現在ログインしているユーザーのアバター画像にアタッチするためのメソッドです。
=end
