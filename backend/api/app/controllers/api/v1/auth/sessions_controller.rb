...
このメソッドは Devise Token Auth が、認証情報が正しくないために認証に失敗した場合に使用します。コントローラにオ
ーバーライドしたメソッドは、 ユーザが認証情報に不備があってサインインに失敗した際にカスタムレスポンスを返します。
------------------------------------------------------------------------------------------------
...
------------------------------------------------------------------------------------------------
**render`が `:json` を引数に取るとき、JSONに変換またはフォーマットするよう指示される。
- JSONはインターネット上で構造化されたデータを送信するために使用される一般的なフォーマットです。異なるシステムが通
信するための、普遍的に理解された言語のようなものです。
- つまり、`render json： data`を実行すると、Railsに「このデータ（@data`）をJSON形式にして、要求があった人に
送り返せ」と伝えていることになります。
=end
