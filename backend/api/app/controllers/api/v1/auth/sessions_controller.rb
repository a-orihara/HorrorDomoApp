...
- このメソッドは、ユーザーのセッション（ログイン状態）を破棄するために使用される。
...
- サインアウト (破棄) に失敗した場合に呼び出されるメソッドは `render_destroy_error`。
------------------------------------------------------------------------------------------------
.devise_token_authのsessions#destroyの挙動
- destroyアクションは、ユーザーのログアウト処理を実行します。
- before_action :set_user_by_tokenにより、現在の認証済みユーザーが@resourceインスタンス変数にセットされま
す。
- @tokenインスタンス変数が@resourceに紐づく認証トークンから取得され、@token.clear!でトークンがクリアされます。
- ユーザーのトークン情報がデータベースから削除されます。user.tokens.delete(client)で対象のクライアントのトー
クンが削除され、user.save!でデータベースに保存されます。
- もしCookieが有効であれば、Cookieも削除されます。
- render_destroy_successメソッドが呼び出され、ログアウト成功のJSONレスポンスがクライアントに返されます。
- ユーザーはログアウト状態になり、以降の認証が必要なリクエストでは、再度ログインが必要になります。
