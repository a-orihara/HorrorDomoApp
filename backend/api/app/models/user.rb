...
class User < ApplicationRecord
  ...
  def unfollow(other_user)
    #  11.2 self.active_relationships.find_by(followed_id: other_user.id).destroy
...
================================================================================================
10
- User`モデルの `follow` メソッドは、デフォルトでは明示的に値を返しません。Rubyでは（そしてRailsのモデルでも）、
最後に評価された式が自動的に返されます
- followメソッドの戻り値は、other_userをfollowing配列に追加した結果となります。これは、成功すれば、新しく追加
されたユーザーを含む、フォローされているユーザーのコレクションが返される
------------------------------------------------------------------------------------------------
<<
...
11.2
. `active_relationships.find_by(followed_id: other_user.id).destroy`
`find_by` は、`find_by(arg1, arg2)` のように、2つの引数を取ることはありません。その代わりに、複数のキーと値の
ペアを指定して検索を絞り込むことができるハッシュ形式の引数を1つ受け取ります。`active_relationships` 内の
`ActiveRecord` オブジェクトのうち、`followed_id` が `other_user` の `id` と一致するものを検索するクエリを
作成する。
- 標準ハッシュ構文**：通常、Rubyのハッシュは中括弧 `{}` で定義され、キーと値のペアには `=>` またはコロン `:`
の構文を使います。例えば、 `{ followed_id: other_user.id }` のようになります。
- メソッドの引数における省略記法ハッシュ構文**： メソッドにハッシュを渡すときに、ハッシュのキーがシンボルである場
合、中括弧を省略して単純にキーと値のペアをメソッドの引数として列挙することができます。
------------------------------------------------------------------------------------------------
Railsで2人のユーザーのフォロー関係を削除する処理。followed_id`が`5`で、`other_user.id`も`5`であるという例で
この操作の仕組みと順序を説明：
------------------------------------------------------------------------------------------------
- ユーザーのフォローを解除するために、このメソッドはまず現在のユーザー(フォロワー)と `other_user` (フォローされ
たユーザー)を結びつける特定のリレーションレコードを特定する必要がある。
------------------------------------------------------------------------------------------------
. **リレーションシップを見つける
- active_relationships.find_by(followed_id: other_user.id)`
- active_relationships`は現在のユーザーがフォロワーとなっているフォロー関係のコレクションを返す。
- find_by(followed_id: other_user.id)` は、 `followed_id` が `other_user` の ID と一致する最初のレコ
ードを検索します。
- 例えば、`other_user.id`が`5`の場合、この部分は現在のユーザーがID `5`のユーザーをフォローしている関係を探す。
------------------------------------------------------------------------------------------------
関係を破壊する
- .destroy`：特定の `Relationship` レコードが見つかったら、そのレコードに対して `.destroy` を呼び出す。
- フォロー関係を削除し、事実上 `other_user` を "フォロー解除" します。destroy`メソッドはレコードを削除するだけ
でなく、destroyアクションに関連するすべてのコールバックを実行し、関連するデータの整合性が維持されるようにします。
