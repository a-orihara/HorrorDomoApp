...
ActiveStorage::Attachmentは、userとavator（登録した画像）の中間テーブル。主となるモデルと
ActiveStorage::Blobとの中間テーブルに相当するモデルです。
ActiveStorage::Blobは、アバター画像の保存先です。アップロードファイルのメタ情報を管理するモデルです。
