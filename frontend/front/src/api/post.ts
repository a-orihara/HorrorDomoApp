...
================================================================================================
4
axios では、`get` メソッドを使うときに、第2引数の axios 設定オブジェクトの `params` プロパティにクエリパラメ
ータを指定して送ることができます。この `params` オブジェクトは、クエリ文字列のパラメータとして URL に追加される。
例えば、
...
は、Axios は `https://example.com/data?key1=value1&key2=value2` に GET リクエストを行います。
------------------------------------------------------------------------------------------------
`searchPosts`
. `client.get` (axios.get)メソッドは最大で2つの引数を受け取ります。第1引数はリクエストを送るURL、第2引数はオ
プション（ヘッダー、パラメーターなど）です。
------------------------------------------------------------------------------------------------
. `searchPosts`の引数`params` がオブジェクト型の理由**:
