...
export const useSignIn = () => {
  ...
  const handleSignIn = async (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    ...
    try {
      ...
      if (res.status === 200) {
        // 2.10 ログインに成功したら、Cookieにアクセストークン、クライアント、uidを保存
...
================================================================================================
2.7
Axiosエラーが発生したが、特定のエラーメッセージがサーバー側で設定されていない等の場合を処理する。これは、ネットワー
クの問題、サーバーの設定エラーなどのシナリオで発生する可能性がある。
------------------------------------------------------------------------------------------------
...
================================================================================================
2.8
このメッセージはAxiosエラーが発生したときに表示されますが、サーバーからの応答オブジェクトはありません。
サーバーがダウンしているか、ネットワークに問題があるか、リクエストがサーバーに到達していないかのいずれかです。
------------------------------------------------------------------------------------------------
...
2.10
**クッキーのキー名に `_` 接頭辞を使う**：
- ウェブ開発では、プライベートな値やシステム固有の値を示すために、特定の変数名やキーの前に `_` を付けるのが一般的。
これは一般的なユーザ定義名と区別するための命名規則です。
- アプリケーションの他の部分やサードパーティのライブラリによって設定されるかもしれない他のクッキー名との衝突を避け
るのに役立ちます。
- このクッキーは内部認証のために使われ、他のスクリプトによって変更されるべきではないことを示唆します。
- これは強制的な慣行ではなく、明瞭さと潜在的な衝突回避のための慣習的なものです。

================================================================================================
3.1
JSで`catch`ブロックがどのように動作するかを理解
. **何が `catch` ブロックのトリガーになるのか**：
- JSでは、`catch` ブロックはエラーが "throw" されたときにトリガーされます。これは `try` ブロック内のコードで何
か問題が発生し、"例外" が発生したときに起こります。
- 例外は JavaScript が生成したエラーオブジェクト (構文エラーやランタイムエラーのようなもの) であることもあれば、
`throw` キーワードを使って手動で投げられたエラーであることもあります。
- `catchブロック`はこれらの例外を "キャッチ "するように設計されているので、エラーがプログラムをクラッシュさせるの
ではなく、潔く例外を処理することができます。
------------------------------------------------------------------------------------------------
. **HTTP ステータスコードと `catch` ブロック**：
- HTTP ステータスコード自体は `catch` ブロックのトリガーにはなりません。例えば、HTTP リクエストのステータスコー
ドが 404 (Not Found) や 400 (Bad Request) であっても、自動的に `catch` ブロックがキャッチするようなエラーは
発生しません。
- しかし、Axios 他のライブラリは、特定の HTTP ステータスコードをエラーとして扱います。例えば、Axios は 2xx の範
囲外のステータスコードをエラーとして扱い、例外をスローします。Axios を使用している場合、サーバーからの 500
(Internal Server Error) 応答が `catch` ブロックをトリガーするのはこのためです。
------------------------------------------------------------------------------------------------
. **`handleSignIn`**：
- handleSignIn` 関数では、Axios を使用して (`signIn` 関数を使用して) HTTP リクエストを行うと、Axios は 500
や400、401のようなステータスコードのレスポンスを受け取った場合、エラーをスローし、`catch`ブロックがこのエラーをキ
ャッチします。
- しかし、HTTP リクエストが成功した場合 (200 ステータスコードなど)、エラーはスローされず、 `catch` ブロックは実
行されません。
------------------------------------------------------------------------------------------------
エラーと例外の違い
. **エラー
- エラーとは、プログラムの実行中に発生する問題で、プログラム自身が対処する能力を備えていないもの。
- 料理をしようとしているときに、重要な材料を切らしていることに気づくようなものです。レシピは期待通りに続けられない。
- JavaScriptでは、存在しない関数を呼び出そうとしたり、定義されていない変数にアクセスしたりと、さまざまな理由でエ
ラーが発生する可能性がある。
- プログラムが対処法を知らない問題に遭遇し、実行を停止する。
------------------------------------------------------------------------------------------------
. **例外
- 例外はエラーの一種であるが、もう少し具体的である。
- JavaScriptでは、`throw`文を使って独自の例外を「投げる」ことができる。これにより、カスタムエラーを作成し、何か
問題が発生したときに特定のメッセージや動作を提供することができます。
- 食材が売り切れるかもしれないことを知っていて、前もって計画を立て、"もしそれがなかったら、代わりに別のものを使おう
"と考えるようなものだ。
- 要するに、すべての例外はエラーだが、すべてのエラーが例外というわけではない。例外とは、あなたが予期して計画したエ
ラーのことであり、一般的なエラーとは、あなたが特に計画しなかった、実行中に発生するその他の問題のことである。
- エラーの際にあらかじめ決められた処理をする。
------------------------------------------------------------------------------------------------
. **catch` ブロック**：
- JavaScriptの `catch` ブロックは、"もしここでエラーや例外が起きたら、私は何をすべきか知っています" と言ってい
るようなものです。セーフティネット。
...
下記理由より、念の為elseを作成。
------------------------------------------------------------------------------------------------
このelseブロックは `signIn` がエラーをスローせずに完了したが、ステータスコードが 200 でない場合に実行されます。
例えば、バックエンドが 202 (Accepted) や 204 (No Content) のようなステータスコードを返した場合、これらはエラ
ーではありませんが、コード内で明示的に処理されていないため、 `else` ブロックが実行されます。
------------------------------------------------------------------------------------------------
このelseブロックはAxios がエラーとして扱わない 200 以外のレスポンスを処理するためのものです。401 のような一般的
なエラーレスポンスでは、処理は直接 `catch` ブロックに移る。
*/