...
ACM証明書を作成する前に、関連するRoute53ゾーン（ホストゾーン）が存在している必要があります。
...
実際の例：
terraform import aws_route53_record.acm_dns_resolve_record Z05<以下ホストゾーンID>__862<以下レコード名>_CNAME
------------------------------------------------------------------------------------------------
...
信のためにSSL/TLS証明書（HTTPS）を使用する場合に相互に関連します。
------------------------------------------------------------------------------------------------
...
- 最後に、`zone_id`は`data.aws_route53_zone.host_zone.zone_id`から取得され、`ttl`は60として設定されます。
これにより、各レコードは特定のRoute 53ホストゾーン内に作成され、TTLは60秒に設定されます。
これにより、ACM証明書のドメイン検証のために必要なRoute 53 DNSレコードが、指定されたホストゾーン内に正確に作成され
ます。
================================================================================================
...
------------------------------------------------------------------------------------------------
上記のコードは、ACMがリクエストされた証明書の検証プロセスを監視し、ドメイン検証が成功するとその情報を持つ Route 53
レコードの FQDN（完全修飾ドメイン名）のリストを受け取ります。検証プロセスが完了すると、Terraformは証明書が有効であ
ると認識し、証明書をリソースに関連付けることができます。これにより、SSL/TLS証明書がフルマネージドな状態で適切にデプ
ロイされることが保証されます。
*/
